<div class="">
                            ...
                            <div class="ml-3 relative" x-data="{ open_drop_down:false }">
                                ...
                                <div x-cloak x-show="open_drop_down" x-on:click="open_drop_down=false" x-on:click.away="open_drop_down=false" class="z-50 origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                                    ...
                                    @if($avaluo->predio)
                                        ...
                                        @if(in_array($avaluo->estado, ['operado']))

                                            <button
                                                wire:click="abrirModalArchivo({{ $avaluo->id }})"
                                                wire:loading.attr="disabled"
                                                class="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                                                role="menuitem">
                                                Actualizar DWG
    ...
    <x-dialog-modal wire:model="modal_archivo" maxWidth="sm">
        ...
        <x-slot name="content">
            ...
            <x-input-group for="archivo" label="Archio archivo" :error="$errors->first('archivo')" class="w-full">

                <div x-data="{ focused: false }" class="w-full">

                    <span class="rounded-md shadow-sm w-full ">

                        <input @focus="focused = true" @blur="focused = false" class="sr-only" type="file" wire:model.live="archivo" id="archivo" accept=".dwg,application/dwg">

                        <label for="archivo" :class="{ 'outline-none border-blue-300 shadow-outline-blue': focused }" class="relative flex items-center justify-between w-full cursor-pointer py-2 px-3 border border-gray-300 rounded-md text-sm leading-4 font-medium text-gray-700 hover:text-gray-500 active:bg-gray-50 active:text-gray-800 transition duration-150 ease-in-out">
                            Selecccione el archivo

                            <div wire:loading.flex wire:target="archivo" class="flex absolute top-1 right-1 items-center">
                                <svg class="animate-spin h-4 w-4 text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
        ...
        <x-slot name="footer">
            ...
            <div class="flex gap-3">
                ...
                <x-button-blue
                    ...
                    <img wire:loading wire:target="guardarArchivo" class="mx-auto h-4 mr-1" src="{{ asset('storage/img/loading3.svg') }}" alt="Loading">
                ...
                <x-button-red
                    wire:click="$toggle('modal_archivo')"
                    wire:loading.attr="disabled"
                    wire:target="$toggle('modal_archivo')"
                    type="button">
