...
<p class="parrafo">
    ...
    @if ($avaluo->predio->superficie_judicial)
        <strong>superficie judicial:</strong>  {{ $avaluo->predio->superficie_judicial }} @if($avaluo->predio->tipo_predio == 1) Metros cuadrados; @else Hectáreas; @endif
    @endif

    @if ($avaluo->predio->superficie_notarial)
        <strong>superficie notarial:</strong> {{ $avaluo->predio->superficie_notarial }} @if($avaluo->predio->tipo_predio == 1) Metros cuadrados; @else Hectáreas; @endif
    @endif

    @if ($avaluo->predio->superficie_total_construccion)

        <strong>Superficie de construcción total:</strong> {{ $avaluo->predio->superficie_total_construccion }} Metros cuadrados;
